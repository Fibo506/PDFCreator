...
public class ITextPdfProcessor : PdfProcessorBase
{
    ...
    private readonly Func<ITextStampAdder> _stampAdderFactory;
    private readonly Func<ITextPageNumbersAdder> _pageNumbersAdderFactory;
    private readonly IFontPathHelper _fontPathHelper;

    // StampAdder needs to be created via container
    public ITextPdfProcessor(IFile file, Func<ITextStampAdder> stampAdderFactory, Func<ITextPageNumbersAdder> pageNumbersAdderFactory, IFontPathHelper fontPathHelper) : base(file)
    {
        _stampAdderFactory = stampAdderFactory;
        _pageNumbersAdderFactory = pageNumbersAdderFactory;
    ...
    public override void AddAttachment(Job job)
    {
        ...
        pdfMerger.AddAttachment(docContext.Document, new ReaderProperties(), job.Profile.AttachmentPage.Files.ToArray());
    ...
    public override void AddStamp(Job job)
    {
        ...
        var stampAdder = _stampAdderFactory();
        stampAdder.AddStamp(docContext.Document, job.Profile);
    ...
    public override void AddPageNumbers(Job job)
    {
        ...
        var pageNumbersAdder = _pageNumbersAdderFactory();
        pageNumbersAdder.AddPageNumbers(docContext.Document, job.Profile);
        WritePdfDocument(docContext);
    }
    ...
    public override void MergePDFs(string targetPdf, string sourcePdf, string pdfOwnerPassword)
    {
        ...
        try
        {
            using PdfReader pdfReader = new PdfReader(documentContext.OriginalFileName, readerProperties);
            using PdfWriter pdfWriter = new PdfWriter(documentContext.TempFileName, writerProperties);
            using var document = new PdfDocument(pdfReader, pdfWriter);
            ITextPdfMerger merge = new ITextPdfMerger();
            merge.AddAttachment(document, readerProperties, sourcePdf);
            document.Close();
    ...
    public override void ApplyPreviewChanges(Job job, IList<PageMapping> pageMappings)
    {
        ...
        try
        {
            ...
            {
                CopyMetadata(targetDocument, sourceDocument);

                // Process only pages that are not excluded and apply rotation if needed
                foreach (var pageMapping in pageMappings)
                {
                    if (pageMapping.IsExcluded)
                        continue;

                    var sourcePageNumber = pageMapping.SourcePageNumber;

                    // Copy the page from source to target
                    sourceDocument.CopyPagesTo(sourcePageNumber, sourcePageNumber, targetDocument);

                    if (pageMapping.RotationAngle != 0)
                    {
                        // Get the page that was just added to the target document
                        var targetPageNumber = targetDocument.GetNumberOfPages();
                        var targetPage = targetDocument.GetPage(targetPageNumber);

                        // Normalizing the rotation just in case
                        var normalizedRotation = pageMapping.RotationAngle % 360;
                        targetPage.SetRotation(normalizedRotation);
                    }
                }
            }
            ...
            File.Move(tempFileName, originalPdfFile);
        }
        catch (Exception ex)
        {
            ...
            _logger.Error(ex, "Error applying preview changes");
            throw new ProcessingException("Failed to apply preview changes", ErrorCode.Preview_FailedToApplyPreviewChanges, ex);
        }
    }
    ...
    private void CopyMetadata(PdfDocument targetDocument, PdfDocument sourceDocument)
    {
        var targetInfo = targetDocument.GetDocumentInfo();
        var sourceInfo = sourceDocument.GetDocumentInfo();

        targetInfo.SetAuthor(sourceInfo.GetAuthor());
        targetInfo.SetCreator(sourceInfo.GetCreator());
        targetInfo.SetKeywords(sourceInfo.GetKeywords());
        targetInfo.SetSubject(sourceInfo.GetSubject());
        targetInfo.SetTitle(sourceInfo.GetTitle());

        var creationDate = sourceInfo.GetMoreInfo("CreationDate");
        if (!string.IsNullOrEmpty(creationDate))
        {
            targetInfo.SetMoreInfo("CreationDate", creationDate);
        }

        targetInfo.SetMoreInfo("ModDate", new PdfDate().ToString());
    }
}
