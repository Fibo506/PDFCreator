...
internal class Program
{
    ...
    private static void RegisterComInterface()
    {
        ...
        {
            CallRegSvr32(GetComDllFilePath(), "/s");
        }
        else
        {
    ...
    private static void UnregisterComInterface()
    {
        if (HasSameDotnetCoreMayorVersion(GetInstalledDotNetCoreFrameworks(), new DotNetVersion(8, 0, 0, 0)))
        {
            CallRegSvr32(GetComDllFilePath(), "/u /s");
        }
        else
        {
            Console.WriteLine("DotNet Core 8 runtime not found");
    ...
    private static string GetComDllFilePath()
    {
        var appDir = GetApplicationDirectory();
        var freeEditionCom = Path.Combine(appDir, "PDFCreator.PDFCreatorCOM.comhost.dll");
        var paidEditionCom = Path.Combine(appDir, "PDFCreator.PDFCreatorProfessionalCOM.comhost.dll");

        if (File.Exists(freeEditionCom))
            return freeEditionCom;

        if (!File.Exists(paidEditionCom))
            Console.WriteLine("WARNING: COM interface DLL not found.");

        // defaulting to paid edition, even if it's not there, it will just fail to register.
        return paidEditionCom;
    }

    private static void CallRegSvr32(string comDll, string parameters)
    {
